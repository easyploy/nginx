...
FROM easyploy/alpine:${BASE_IMAGE_TAG}
...
RUN set -ex; \
    ...
    \
	addgroup -g 1000 -S easyploy; \
	adduser -u 1000 -D -S -s /bin/bash -G easyploy easyploy; \
	sed -i '/^easyploy/s/!/*/' /etc/shadow; \
	echo "PS1='\w\$ '" >> /home/easyploy/.bashrc; \
    \
    ...
    \
    install -g easyploy -o easyploy -d \
        "${APP_ROOT}" \
        "${FILES_DIR}" \
        /etc/nginx/conf.d \
        /var/cache/nginx \
    ...
    chown -R easyploy:easyploy /etc/nginx; \
    \
    ...
    echo "find ${APP_ROOT} ${FILES_DIR} -maxdepth 0 -uid 0 -type d -exec chown easyploy:easyploy {} +" > /usr/local/bin/init_volumes; \
    ...
    \
    chown easyploy:easyploy /usr/share/nginx/html/50x.html; \
    \
